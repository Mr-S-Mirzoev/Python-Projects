...
class SimpleTraining:
    def __init__(self):
        ...
        window.modify_bg(Gtk.StateType.NORMAL, setColor(248, 177, 149))
        window.connect("delete-event", self.on_destroy)
        window.show()
...
class GenderTraining:
    ...
    def set_new(self):
        ...
        self.which = self.buff.list[int(random() * len(self.buff.list))]
        self.builder.get_object("genderTrLabelWord").set_text(self.which["German"][3:].strip())
        self.builder.get_object("genderTrEvBox3").modify_bg(Gtk.StateType.NORMAL, setColor(248, 177, 149))
...
class MatchTraining:
    ...
    def set_new(self):
        ...
        self.builder.get_object("matchTrEvBox8").modify_bg(Gtk.StateType.NORMAL, setColor(248, 177, 149))
...
class PhraseTraining:
    def __init__(self):
        ...
        self.filter = lambda x: x["Type"] == "Phrase"
        self.q = get_quantity_f(self.filter)
        self.buff = fBuffer([0, 100], self.filter)
        self.which = self.buff.list[int(random() * len(self.buff.list))]
        self.dividedPhrase = self.which["German"].strip().split()
        self.distr_to_butts()
        for i in range(16):
            self.builder.get_object("phraseButton" + str(i)).connect("clicked", self.click_on_but, "phraseButton" + str(i))

        window = self.builder.get_object("phraseWindow")
        window.modify_bg(Gtk.StateType.NORMAL, setColor(248, 177, 149))
        window.connect("delete-event", self.on_destroy)
        window.show()
    ...
    def set_new(self):   
        ...
        #self.builder.get_object("checkPhraseLabel").hide()
        self.builder.get_object("checkPhraseLabel").set_text(str())
        self.builder.get_object("checkPhraseLabel").modify_bg(Gtk.StateType.NORMAL, setColor(248, 177, 149))
    ...
    def distr_to_butts(self):
        tmp = list(range(0, len(self.dividedPhrase)))
        shuffle(tmp)

        for i in range(len(self.dividedPhrase), 16):
            self.builder.get_object("phraseButton" + str(i)).hide()

        for i in range(len(self.dividedPhrase)):
            but = self.builder.get_object("phraseButton" + str(i))
            but.set_label(self.dividedPhrase[tmp[i]])
            but.show()

    def click_on_but(self, widget, ind):
        ...
        if len(self.builder.get_object("phraseLabel").get_text().strip()) == len(self.which["German"].strip()):
            self.check()
            #self.builder.get_object("checkPhraseLabel").show()
            GLib.timeout_add(1500, self.set_new)
    
    def check(self):
        if self.builder.get_object("phraseLabel").get_text() == self.which["German"].strip():
            self.which["Attributes"] = self.set_attr(self.which["Attributes"], True)
...
class Main:
    def __init__(self):
        ...
        self.builder.get_object("mainButtonStartTr3").connect("clicked", self.startPhraseTraining)
    ...
    def startPhraseTraining(self, widget):
        PhraseTraining()
